...
def register():
    ...
    if form.validate_on_submit():
        try:
            ...
            response['errors']['username'] = [errorInfo[1]]
            return jsonify(dict(meta=dict(code=400), response=response)), 400
...
def confirm_email(token):
    ...
    if invalid or (expired and not already_confirmed):
        ...
        if invalid:
            ...
            return redirect("http://www.frontend.com/get?message=Token_invalid", code=302)
        else:
            #code=202
            #response={"errors": {"token":"Expired"}}
            return redirect("http://www.frontend.com/get?message=Token_expired", code=302)
        #return jsonify(dict(meta=dict(code=code), response=response)), code
    ...
    #                    get_url(_security.post_login_view))
        ...
        #return jsonify(dict(meta=dict(code=code), response=response)), code
    return redirect("http://www.frontend.com/get?message=Email_confermata", code=302)
